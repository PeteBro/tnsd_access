...
```python
from tnsd_access import init_dataset
...
# Load all matching trials into memory — returns an mne.Epochs object
epochs = loader.get_data(trials)
# epochs.get_data() shape: (n_trials, n_channels, n_samples)
# epochs.metadata has one row per trial
```
...
# epochs.get_data() shape: (n_conditions, n_channels, n_samples)
# epochs.metadata has one row per condition
